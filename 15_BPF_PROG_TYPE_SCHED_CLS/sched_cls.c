/* Userspace-программа для проверки BPF_PROG_TYPE_SCHED_CLS (скелет libbpf) */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
...
int main(int argc, char **argv)
{
    ...
    /* Присоединение через tc */
    prog_fd = bpf_program__fd(skel->progs.tc_cls_count);
    ...
    if (system("tc filter add dev lo egress bpf da object-pinned " PIN_PATH) != 0) {
