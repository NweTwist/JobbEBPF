/* Userspace-программа для проверки BPF_PROG_TYPE_SK_SKB (скелет libbpf) */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
...
int main(void)
{
    ...
    /* Сначала присоединяем SK_SKB к sockmap, затем добавляем сокеты.
    ...
    prog_fd = bpf_program__fd(skel->progs.sk_skb_verdict);
    err = bpf_prog_attach(prog_fd, sockmap_fd, BPF_SK_SKB_STREAM_VERDICT, 0);
    if (err) {
        fprintf(stderr, "  [ATTACH] ОШИБКА: bpf_prog_attach: %s\n", strerror(errno));
        goto out;
    }

    /* Добавление принимающего сокета в sockmap после присоединения программы.
     * SK_SKB stream_verdict вызывается при получении данных на сокете из sockmap */
    __u32 idx = 0;
    err = bpf_map_update_elem(sockmap_fd, &idx, &acc_fd, BPF_ANY);
    if (err) {
    ...
    /* Отправка данных (данные поступают на acc_fd, вызывая stream_verdict) */
    char buf[64] = "test_sk_skb_data";
    for (int i = 0; i < 3; i++) {
